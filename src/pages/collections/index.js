...
function Collection({ products }) {
  ...
  const handlePriceChange = (label) => {
    ...
    if (isOptionSelected) {
      updatedSelectedPriceOptions = selectedPriceOptions.filter((selectedOption) => selectedOption !== label)
    } else {
  ...
  useEffect(() => {
    ...
    const filterProductsBySupplier = () => {
      ...
      return products.filter((product) => selectedSuppliers.includes(product.supplier.name))
    }

    const priceFilteredProducts = filterProductsByPrice()
    const supplierFilteredProducts = filterProductsBySupplier()
    const finalFilteredProducts = priceFilteredProducts.filter((product) => supplierFilteredProducts.includes(product))
  ...
  return (
    <>
      <Filter
        ...
        itemSuplier={['Nhật Bản', 'Việt Nam', 'Trung Quốc', 'Thái Lan', 'Hoa Kỳ']}
        itemPrice={[
          'Dưới 1.000.000₫',
          '1.000.000₫ - 2.000.000₫',
          '2.000.000₫ - 3.000.000₫',
      ...
      <div className='container'>
        <p className='text-danger'>
          Tùy chọn giá:
          <span className='text-success mx-2'>{selectedPriceOptions.join(' , ')}</span>
          <br />
          Tùy chọn thương hiệu:
          <span className='text-success mx-2'>{selectedSuppliers.join(' , ')}</span>
        </p>
      </div>
              ...
              filteredProducts.map((product) =>
                product.stock > 0 ? (
                  <div className='col-md-3 col-sm-4 col-xs-6' key={product._id}>
                    <Card id={`/productDetail/${product._id}`} products={product} handleAddToCart={handleAddToCart} />
                  </div>
                ) : null
              )
            ) : (
